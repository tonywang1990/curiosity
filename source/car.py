...
import picar
import logging
from source.primitive import Target, FrameSize, Context
from source.vision import Cortex, ODConfig
import time
from collections import deque
from enum import Enum
from source.pid import PID
...
class Camera:
    ...
    def track_target(self, target: Target) -> Tuple[int, int]:
        ...
        error = target.center[0] - \
            frame_center[0], target.center[1] - frame_center[0]
        update = self._pan_control(error[0]), self._tilt_control(error[1])
        logger.info(
            f"Camera: frame center {frame_center}, target center {target.center}, error {error}, update {update}")
        return update
    ...
    def follow_target(self, context: Context) -> Context:
        ...
        if target is None:
            logger.warning(
                f"{self.__class__}: target is None, maintain currnet angle..")
        else:
            update = self.track_target(target)
            logger.info(
                f"pan: {self.servo.pan}, pan_change: {update[0]}, tilt: {self.servo.tilt}, pan_change: {update[1]}")
...
class FrontWheels:
    def __init__(self, turning_offset: int = 0, max_turn: int = 35):
        self._servo = Servo.Servo(
            channel=0, bus_number=1, offset=turning_offset)
    ...
    def follow_target(self, context: Context) -> Context:
        try:
            if context.camera_pan and context.speed:
                ...
                self.turn = turn
                logger.info(
                    f"FrontWheels: direction {context.speed}, turn {self.turn}")
            else:
                logger.warning(
                    f'FrontWheels: "camera_pan" not found in info, do nothing.. \n {context}')
...
class BackWheels:
    ...
    def track_target(self, context: Context, goal_ratio: float) -> int:
        ...
        if frame_size and target:
            ...
            logger.info(
                f"Backwheels: actual_size {actual_size}, goal_size {goal_size}, error {error}, speed {self.speed}")
        else:
            logger.warning(
                f'Backwheels: no frame_size and target found, stoping.. \n {context}')
            speed = 0
        return speed
...
class Car:
    def __init__(self):
        ...
        self.controllers = {'camera_pan': self.camera._pan_control,
                            'camera_tilt': self.camera._tilt_control, 'back_wheels': self.back_wheels._control}
        ...
        try:
            for _ in range(max_iter):
                if not camera.isOpened():
                    logger.warning(
                        "camera.isOpened() == False, exiting loop..")
                    break
